...
rollback_verify() {
  ...
  # Use TEST_DIR if in test mode, otherwise check system directories
  local dirs_to_check=()
  if [[ -n "${TEST_DIR:-}" ]]; then
    # Test mode - check test directories only
    dirs_to_check=(
      "${TEST_DIR}/opt/vps-provision"
      "${TEST_DIR}/var/vps-provision"
    )
  else
    # Production mode - check system directories
    dirs_to_check=(
      "/opt/vps-provision"
      "/var/vps-provision"
    )
  ...
  # Check for residual users (skip in test mode)
  if [[ -z "${TEST_DIR:-}" ]]; then
  ...
  # Check for residual services (skip in test mode or if systemctl not available)
  if [[ -z "${TEST_DIR:-}" ]] && command -v systemctl &>/dev/null; then
    local services_to_check=("xrdp" "lightdm")
    local service
    for service in "${services_to_check[@]}"; do
      # Only check if service unit exists
