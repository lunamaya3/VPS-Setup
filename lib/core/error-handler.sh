...
error_classify() {
  ...
  # Timeout errors (check FIRST - exit code 124 is definitive, also check for timeout messages)
  if [[ "$exit_code" -eq 124 ]]; then
    echo "$E_TIMEOUT"
    return 0
  fi

  if echo "$combined" | grep -qiE "(connection timed out|operation timed out|timeout)"; then
    echo "$E_TIMEOUT"
    return 0
  fi

  # Network-related errors (but not timeouts which were handled above)
  if [[ "$exit_code" -eq 100 ]] ||
...
error_get_suggestion() {
  local error_type="$1"
  local suggestion="${ERROR_SUGGESTIONS[$error_type]:-Review error details and consult documentation.}"
  echo "$suggestion"
}
...
execute_with_retry() {
  ...
  while [[ $attempt -lt $max_retries ]]; do
    ...
    # Execute command and capture output (combined stdout/stderr)
    ...
    if error_exit_code_whitelisted "$exit_code"; then
      ...
      echo "$output" # Pass through command output for testing/verification
      return 0
    fi
    ...
    log_warning "Command failed with error type: $error_type (severity: $severity, exit_code: $exit_code)"

    # Critical errors (FATAL severity) should not be retried
    if [[ "$severity" == "FATAL" ]]; then
      log_error "Critical error detected, aborting retries"
      local suggestion
      suggestion=$(error_get_suggestion "$error_type")
      echo "Critical error: $cmd failed with $error_type"
      echo "$suggestion"
